...
async function main() {
  ...
  const announcement1 = await prisma.announcement.create({
    data: {
      ...
      content:
        "Excited to have everyone on board. Let's build something great together!",
      isPinned: true,
      authorId: user1.id,
      workspaceId: workspace.id,
    },
  ...
  const announcement2 = await prisma.announcement.create({
    data: {
      ...
      content:
        "Join us for the quarterly planning session next Monday. All hands meeting at 10am.",
      isPinned: false,
      authorId: user1.id,
      workspaceId: workspace.id,
    },
  ...
  await prisma.mention.createMany({
    data: [{ userId: user2.id, commentId: comment2.id }],
  });
  ...
  await prisma.goalUpdate.createMany({
    data: [
      {
        content:
          "Completed design phase on schedule. Moving to development next week.",
        goalId: goal1.id,
        authorId: user1.id,
      },
      {
        content:
          "Development is 60% complete. On track for milestone delivery.",
        goalId: goal1.id,
        authorId: user1.id,
      },
    ],
